...
public class TreeServiceImpl<M extends BaseMapper<T>, T extends TreeEntity> extends CrudServiceImpl<M, T> implements ITreeService<T> {
	...
	public void convertTreeSort(List<T> sourceList, List<T> targetList, String parentId) {
		...
		.forEach(tree -> {
			...
			sourceList.stream().filter(
					child -> child.getParentId() != null && child.getParentId().equals(tree.getPkValue()))
	...
	public boolean updateTreeLeaf(T entity) {
		...
		sql.append(" where status='0'").append(" and parent_id = '").append(entity.getPkValue()).append("'");
		sql.append(" ) a )");
	...
	public boolean deleteChild(T entity) {
		...
		queryWrapper.like("parent_ids", entity.getPkValue());
	...
	public boolean saveOrUpdate(T entity) {
		...
		if (parentEntity == null || StringUtils.isBlank(entity.getParentId()) || TreeEntity.ROOT_ID.equals(entity.getParentId())) {
			...
			// 设置保存对象的父节点
			entity.setParentId(parentEntity.getPkValue());
		}
		//设置父类对象
		...
		entity.setParentIds(parentEntity.getParentIds() + parentEntity.getPkValue() + ",");
		...
		if (oldEntity != null 
			...
			parentEntity.setPkValue(oldEntity.getParentId());
			this.updateTreeLeaf(parentEntity);
		}
		return true;
	}
