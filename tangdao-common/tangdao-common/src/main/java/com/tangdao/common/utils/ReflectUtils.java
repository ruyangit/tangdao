			...
			for (int i=0; i<cs.length; i++){
				if (args[i] != null && !args[i].getClass().equals(cs[i])){
					if (cs[i] == String.class){
						...
						args[i] = Integer.valueOf(String.valueOf(args[i]));
					}else if (cs[i] == Long.class){
						args[i] = Long.valueOf(String.valueOf(args[i]));
					}else if (cs[i] == Double.class){
						args[i] = Double.valueOf(String.valueOf(args[i]));
					}else if (cs[i] == Float.class){
						args[i] = Float.valueOf(String.valueOf(args[i]));
					}else if (cs[i] == Date.class){
						if (args[i] instanceof String){
							args[i] = DateUtils.parseDate(String.valueOf(args[i]), parsePatterns);
						}
					}
				}
			}
