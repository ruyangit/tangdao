...
public class SmsWaitPacketsListener extends AbstartRabbitListener {
    ...
    private void asyncSendTask(MobileCatagory mobileCatagory) {
        ...
        task.setProcessStatus(StringUtils.isEmpty(task.getErrorMessageReport())
                              || ListUtils.isEmpty(task.getPackets()) ? PacketsProcessStatus.PROCESS_COMPLETE.getCode() : PacketsProcessStatus.PROCESS_EXCEPTION.getCode());
    ...
    private void returnFeeToUser(SmsMtTask task, MobileCatagory mobileCatagory) {
        if (task.getReturnFee() != null && task.getReturnFee() != 0 && mobileCatagory != null) {
            logger.info("用户：{} 发送短信 存在错号：{}个，重复号码：{}个，单条计费：{}条，共扣费：{}条，共需返还{}条", task.getUserCode(),
                        ...
                        task.getReturnFee());
            try {
                userBalanceService.updateBalance(task.getUserCode(), task.getReturnFee(),
                                                 PlatformType.SEND_MESSAGE_SERVICE.getCode(),
                                                 PaySource.USER_ACCOUNT_EXCHANGE, PayType.SYSTEM, 0d, 0d, "错号或者重号返还",
                                                 false);
            } catch (Exception e) {
                logger.error("返还用户：{}，总短信条数：{} 失败", task.getUserCode(), task.getMobiles().length * task.getFee(), e);
            }
        }
    }
    ...
    private MobileCatagory findOutMatchedMobiles() {
        // 转换手机号码数组
        List<String> mobiles = new ArrayList<>(Arrays.asList(smsMtTaskLocal.get().getMobile().split(MobileCatagory.MOBILE_SPLIT_CHARCATOR)));
        ...
        if (ListUtils.isNotEmpty(blackMobiles)) {
            // 移除需要执行的手机号码
    ...
    private UserSmsConfig getSmsConfig() {
        ...
        UserSmsConfig userSmsConfig = userSmsConfigService.getByUserCode(smsMtTaskLocal.get().getUserCode());
        if (userSmsConfig == null) {
            userSmsConfigService.save(smsMtTaskLocal.get().getUserCode(), UserBalanceConstant.WORDS_SIZE_PER_NUM,
    ...
    private void joinTaskPackets(String mobile, SmsPassageAccess passageAccess) {
        ...
        if (passageAccess == null || StringUtils.isNotEmpty(smsMtTaskPackets.getRemarks())
        ...
        smsMtTaskPackets.setUserCode(task.getUserCode());
    ...
    private void checkContentSignature(UserSmsConfig smsConfig) {
        ...
        if (smsConfig.getSignatureSource() != null
            ...
            if (!PatternUtils.isContainsSignature(smsMtTaskLocal.get().getContent())) {
    ...
    private void markContentHasSensitiveWords() {
        ...
        if (ListUtils.isNotEmpty(filterWords)) {
            smsMtTaskLocal.get().setForbiddenWords(StringUtils.join(filterWords,
                                                                    MobileCatagory.MOBILE_SPLIT_CHARCATOR));
    ...
    private boolean isSmsPassageAccessAvaiable(SmsPassageAccess access) {
        return access != null && access.getStatus() != null && Integer.valueOf(access.getStatus()) == PassageStatus.ACTIVE.getValue();
    }
    ...
    private void loadSmsTemplateByContent(UserSmsConfig smsConfig) {
        ...
        // 短信是否免审
        if (smsConfig.getMessagePass().equals(UserSettingsContext.SmsMessagePass.NO.getValue())) {
            ...
            template = smsTemplateService.getByContent(smsMtTaskLocal.get().getUserCode(),
                                                       smsMtTaskLocal.get().getContent());
            if (template == null) {
    ...
    private boolean isSameMobileOutOfRange(UserSmsConfig smsConfig) {
        ...
        // 根据userId获取白名单手机号码数据
        Set<String> whiteMobiles = smsMobileWhiteListService.getByUserCode(smsMtTaskLocal.get().getUserCode());
        ...
        for (String mobile : mobiles) {
            ...
            if (ListUtils.isNotEmpty(whiteMobiles) && whiteMobiles.contains(mobile)) {
                continue;
            }

            // 判断短信发送是否超速
            int beyondExpected = smsMobileTablesService.checkMobileIsBeyondExpected(smsMtTaskLocal.get().getUserCode(),
        ...
        if (ListUtils.isNotEmpty(benyondSpeedList)) {
            // 移除需要执行的手机号码
        ...
        if (ListUtils.isNotEmpty(benyondTimesList)) {
            // 移除需要执行的手机号码
        ...
        return ListUtils.isNotEmpty(mobiles);
    }
    ...
    private void doExceptionOverWithReport(List<String> mobiles, String remarks) {
        ...
        submit.setUserCode(task.getUserCode());
        ...
        if (task.getAppType() != null && AppType.DEVELOPER.getCode() != task.getAppType()) {
            ...
            PushConfig pushConfig = pushConfigService.getPushUrl(task.getUserCode(), CallbackUrlType.SMS_STATUS.getCode(),
                                                                 task.getCallback());

            // 推送信息为固定地址或者每次传递地址才需要推送
            if (pushConfig != null && SettingsContext.PushConfigStatus.NO.getCode() != Integer.valueOf(pushConfig.getStatus())) {
    ...
    private void subpackage(SmsRoutePassage routePassage) {
        ...
        for (String passageId : routePassage.getPassageMobiles().keySet()) {
            ...
            if (ListUtils.isEmpty(groupMobiles)) {
                continue;
            }
