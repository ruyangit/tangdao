 ...
 *
 */
package com.tangdao.exchanger.resolver;
...
public class HttpClientManager {
	...
	private static CloseableHttpClient getHttpClient(String url, Integer maxTotal, Integer maxPerRoute) {
		...
		if (!LOCAL_HTTP_CLIENT_FACTORY.containsKey(hostAndPort)) {
			synchronized (SYNC_LOCK) {
				...
				log.info("Http url[" + url + "] initial succeed, args - maxTotal[" + maxTotal + "], maxRoutePerHost ["
						+ maxPerRoute + "]");
			}
		}
	...
	public static String get(String url, Map<String, Object> params) {
		...
		HttpGet httpget = new HttpGet(StrUtil.isEmpty(parameters) ? url : url + "?" + parameters);
	...
	private static void setHttpParameters(HttpPost httpPost, Map<String, Object> params, String encoding) {
		...
		try {
			...
			log.error("Encoding http form entity failed", e);
		}
	}
	...
	public static String post(String url, Map<String, Object> headers, Map<String, Object> params, Integer maxTotal,
		...
		try {
			...
			log.info("URL：{} 请求耗时：{} ms", url, System.currentTimeMillis() - startTime);
			// 释放资源
	...
	public static String postJson(String url, Map<String, Object> headers, String jsonBody, Integer maxTotal,
		...
		if (StrUtil.isEmpty(jsonBody)) {
			throw new BusinessException("用户JSON报文为空");
		}
		...
		try {
			...
			if (statusCode != 200) {
				// httpPost.abort();
				log.error("Http回执状态码异常，statusCode : {}", statusCode);
			}
